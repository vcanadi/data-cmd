...
data TΣ = TΣ TC TΠ deriving (Show, Eq)
...
instance {-# OVERLAPS #-} HasTR a => HasTR [a] where aTR xs = T . TΣ (TC "L") . TΠ <$> traverse aTR xs ## "List" -- ^ List is product of n fields instead of binary constructed object
instance {-# OVERLAPPABLE #-} (Generic a, GTR (Rep a)) => HasTR a where aTR x = genTR x ## "Gen"
...
instance GTRΣ f => GTR (D1 m f) where gTR (M1 x) = T <$> gTRΣ False x ## "T"

-- | "Generic Tree Renderer" logic on generic sum type
class GTRΣ (f :: Type -> Type)                     where gTRΣ :: Bool -> f p -> Res TΣ
instance (GTRΣ f, GTRΣ g)        => GTRΣ (f :+: g) where gTRΣ _ (L1 x) = gTRΣ True x ## "LHS"; gTRΣ _ (R1 x) = gTRΣ True x ## "RHS"
instance (GTRΠ f, Constructor m) => GTRΣ (C1 m f)  where gTRΣ _ (M1 x) = TΣ (TC $ conName @m Dummy) <$> gTRΠ x ## "C"

-- | "Generic Tree Renderer" logic on generic product type
class GTRΠ (f :: Type -> Type)              where gTRΠ :: f p -> Res TΠ
instance (GTRΠ f, GTRΠ g) => GTRΠ (f :*: g) where gTRΠ (x:*:y) = (gTRΠ x ## "FST") <> (gTRΠ y ## "SND")
instance HasTR a => GTRΠ (S1 m (Rec0 a))    where gTRΠ (M1 (K1 x)) = TΠ . pure <$> aTR x ## "Type"
instance GTRΠ U1                            where gTRΠ _ = pure $ TΠ []
