...
instance {-# OVERLAPS #-} HasFP String where aFP  = readF >>> (#< "Parsing String")
instance {-# OVERLAPS #-} HasFP a => HasFP [a] where aFP ("L" :.. cs) = traverse (aFP @a) cs #< "Parsing list"
                                                     aFP _            = parseErr "Error parsing list"
instance {-# OVERLAPPABLE #-} (Generic a, GFP (Rep a), GTypNm (Rep a)) => HasFP a where aFP = genFP >>> (#< "Parsing Gen")
...
instance (GFPΣ f, Datatype m) => GFP (D1 m f) where gFP fo = M1 <$> gFPΣ @f fo #< ("Datatype: " <> datatypeName @m Dummy)
...
instance (GFPΣ f, GFPΣ g)        => GFPΣ (f :+: g) where gFPΣ t                                               = ((L1 <$> gFPΣ @f t #+< "L1 selection") <|> (R1 <$> gFPΣ @g t #+< "R1 selection")) #+< "of"
instance (GFPΠ f, Constructor m) => GFPΣ (C1 m f)  where gFPΣ (FΣ (FC c) ts) | conName @m Dummy `caseInvEq` c = M1 <$> gFPΠ @f ts #+< "Constructor " <> conName @m Dummy <> " matched"
...
instance (GFPΠ f, GFPΠ g, MW f) => GFPΠ (f :*: g)      where gFPΠ (FΠ ts)  = (:*:) <$> (gFPΠ @f (FΠ $ take (mW (Proxy @f)) ts) #*< "FST")
                                                                                   <*> (gFPΠ @g (FΠ $ drop (mW (Proxy @f)) ts) #*< "SND")
instance (HasFP a, Selector m) => GFPΠ (S1 m (Rec0 a)) where gFPΠ (FΠ [t]) = M1 . K1 <$> aFP @a t #*<  "Selector --" <> selName @m Dummy <> "-- matched"
                                                             gFPΠ t        = parseErr  $ "Selector " <> show t <> " not matched with expected selector " <> selName @m Dummy
